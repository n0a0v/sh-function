...
#include <sh/copyable_function.hpp>

using sh::copyable_function;
...
TEST(sh_copyable_function, ctor_default)
{
	copyable_function<int(int)> x;
	EXPECT_FALSE(bool(x));
	EXPECT_EQ(x, nullptr);
}
TEST(sh_copyable_function, ctor_nullptr)
{
	copyable_function<int(int)> x(nullptr);
	EXPECT_FALSE(bool(x));
	EXPECT_EQ(x, nullptr);
}
...
{
	bool called = false;
	copyable_function<void(bool&)> x{ [](bool& called) { called = true; } };
	x(called);
	EXPECT_TRUE(called);
}
TEST(sh_copyable_function, func)
{
	copyable_function<int(int)> x(plus_1);
	ASSERT_TRUE(bool(x));
	ASSERT_NE(x, nullptr);
	EXPECT_EQ(x(0), 1);
}
TEST(sh_copyable_function, func_ptr)
{
	copyable_function<int(int)> x(&plus_1);
	ASSERT_TRUE(bool(x));
	ASSERT_NE(x, nullptr);
...
TEST(sh_copyable_function, functor_const)
{
	...
	copyable_function<int(int)> x(plus_2{});
	ASSERT_TRUE(bool(x));
	ASSERT_NE(x, nullptr);
	EXPECT_EQ(x(0), 2);
}
...
TEST(sh_copyable_function, lambda_mutable)
{
	...
	const copyable_function<int(int)> x(std::move(plus_3));
	ASSERT_TRUE(bool(x));
	ASSERT_NE(x, nullptr);
	EXPECT_EQ(x(0), 3);
}
